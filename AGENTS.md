...
| [docs/background.md](docs/background.md) | Touching the page background (`GalaxyBackground`, `src/components/galaxy/`: the shader, its palette or `featureScale`), how it scrolls with the page in tiles (`tilePlan`, `createPainter`), how it covers the screen (`overdrawSize`), or its fallback colour |
