...
| [docs/card-layout.md](docs/card-layout.md) | Touching how cards sit on `/d/…` (the tilt on hover, `randomTilt`/`leanOf`/`untiltedBox` in `src/components/tilt.ts`), the plan track's padding, or dragging cards in the plan to reorder it (the grip, `PlanCard`, `Reorder`) |
